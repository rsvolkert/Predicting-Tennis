...
- Project based off a paper by Paul K. Newton and Joseph B. Keller called Probability of Winning at Tennis 1. Theory and Data
...
- Both two out of three and three out of five set matches were considered aloowing for 13-point tiebreaker in each set if needed
...
Data used in paper: 
  ...
  - Men and Women semifinalists in the 2002 Wimbledon tournament
  
Data used in our simulation:
  
  - Men and Women semifinalists in the 2019 Wibledon tournament
...
## 2002 U.S Open: Women
```{r, echo=FALSE}
Tourney1_Women = data.frame(`P(Winning a Rally)` = TDWomen[,1],
                            `Paper P(Winning the Tournement)` = c(.2872, .3599, .0482, .3074))
#`Our P(Winning the Tournement )` =round( pTC(TDWomen[1,1],TDWomen[2,1],TDWomen[3,1],TDWomen[4,1], 2), 4)
 
rownames(Tourney1_Women)<- c("S. Williams","L. Davenport",  "A. Mauresmo", "V. Williams")


T1<- table(base::sample(rownames(Tourney1_Women), 2000, prob = Tourney1_Women$Paper.P.Winning.the.Tournement., replace = TRUE)) 
...
ggplot(data.frame(T1)) +
  ...
  xlab("Players") + 
  ylab("P(Winning the Tournement") +
  ggtitle("Which Player Won the Tournement?")
...
```{r,echo=FALSE, fig.align='center'}
include_graphics("Serena.png")
```

## 2002 U.S Open: Men
```{r, echo=FALSE, }
Tourney1_Men = data.frame(`P(Winning a Rally)` = TDMen[,1],
                            `Paper P(Winning the Tournement)` = c(.6747, .0851, .0945, .1457))
#`Our P(Winning the Tournement )` =round( pTC(TDMen[1,1],TDMen[2,1],TDMen[3,1],TDMen[4,1], 2), 4)
 
rownames(Tourney1_Men)<- c("P. Sampras","S. Schalken",  "A. Agassi", "L. Hewitt")


T1<- table(base::sample(rownames(Tourney1_Men), 2000, prob = Tourney1_Men$Paper.P.Winning.the.Tournement., replace = TRUE)) 
...
WimWomen <- read_csv("2019 Womens Wimbledon.csv")

WimWomen$`P(Winning a Rally)` <- WimWomen$`Points Won on Serve`/WimWomen$`Total Points Served`

WimWomen <- as.data.frame(WimWomen)

WimWomen$`P(Winning Tournement)`<- pTC(WimWomen[1,4][1], WimWomen[2,4][1],WimWomen[3,4][1],WimWomen[3,4][1],2)


T1<- table(base::sample(WimWomen$Name, 2000, prob = WimWomen$`P(Winning a Rally)`, replace = TRUE)) 
...
```{r, warning=FALSE, echo=FALSE, message=FALSE}
WimMen <- read_csv("2019 Men's Wimbledon.csv")

WimMen <- as.data.frame(WimMen)

WimMen$`P(Winning a Rally)` <- WimMen$`Points Won on Serve`/ WimMen$`Total Points on Serve`



WimMen$`P(Winning Tournement)`<- pTC(WimMen[1,4][1], WimMen[2,4][1],WimMen[3,4][1],WimMen[3,4][1],2)


T1<- table(base::sample(WimMen$Name, 2000, prob = WimMen$`P(Winning a Rally)`, replace = TRUE)) 
