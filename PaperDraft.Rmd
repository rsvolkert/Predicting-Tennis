...
$$p^T_i =  \boldsymbol{P}_{ij}(\boldsymbol{P}_{ik}\boldsymbol{P}_{kl} + \boldsymbol{P}_{il}\boldsymbol{P}_{lk} ) $$
Note that $\sum_{\forall i} p^T_i =1$. We will sample 1 player from the 4 semifinalists with the probabilities calculated above to determine the tournament winner. 
...
First we will consider data from the 2002 US Open for both men's and women's. We want to reproduce the probability of winning a game. Table 1 compares the empirical probability of winning a game versus the theoretical probability of winning a game calculated in @Tennis versus the probability of winning a game that we calculated for the 4 semi finalists in this tournament. We can see that the vales that we calculated and the values calculated in @Tennis are the same. These values do differ slightly from the empirical probabilities. Table 2 outlines the same comparisons except for the men's 2002 US Open. 



Next, we want to compare the probability of winning a tournament. @Tennis does not provide their predictions of tournament winners, only their probabilities compared to what actually happened in the tournament. The paper also only considers the 4 semifinalists when predicting the tournament, so we will as well. 
...
```{r, echo=FALSE, message=F, warning=F, fig.pos="H", fig.cap = "2002 Women's US Open"}
...
We fist look at 2002 US Open's Women's tournament for the 4 semi finalists. We calculate the probability each semi finalist has of winning the tournament, then we sample one player to win the tournament with the probabilities we calculated. Figure 1 shows the results from 2000 tournaments played with these 4 semi finalists. From our simulations we predict that S. Williams wins the tournament, and in fact she did win this tournament. 


Similarly, we want to consider the 2002 US Open for the men's semifinalists. Using the same method outline above, we can see in Figure 2 that we predict P. Sampras to win the tournament, and in fact he did win this tournament. 

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.pos="H", fig.cap = "2002 Men's US Open"}
...
```
```{r, message=F, warning=F, fig.pos="H", fig.cap = "2002 Women's Wimbledon"}
...
```
```{r, message=F, warning=F, fig.pos="H", fig.cap = "2002 Men's Wimbledon"}
...
```{r, warning=FALSE, echo=FALSE, message=F, fig.pos="H", fig.cap = "2019 Women's Wimbledon"}
...
```{r, warning=FALSE, echo=FALSE, message=F, fig.pos="H", fig.cap = "2019 Men's Wimbledon"}
